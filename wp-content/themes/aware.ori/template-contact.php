...
if($_REQUEST['c_submitted']){
	...
	if(trim($_REQUEST['c_name'] == "")){
		//it's empty
		$name_error = __('<!--:es-->Você esqueceu de preencher o seu nome<!--:--><!--:pb-->Você esqueceu de preencher o seu nome<!--:-->', 'framework');
		$error = true;
	}else{
		//its ok
	...
	if(trim($_REQUEST['c_email'] === "")){
		//it's empty
		$email_error = __('<!--:es-->Você esqueceu de preencher o seu e-mail<!--:--><!--:pb-->Você esqueceu de preencher o seu e-mail<!--:-->', 'framework');
		$error = true;
	}else if(!is_email( trim($_REQUEST['c_email'] ) )){
		//it's wrong format
		$email_error = __('<!--:es-->Formato e-mail errado<!--:--><!--:pb-->Formato e-mail errado<!--:-->', 'framework');
		$error = true;
	}else{
		//it's ok
	...
	if ($captcha == 'on') {
		if(trim($_REQUEST['c_captcha'] !== "4")){
			//it's empty
			$captcha_error = __('<!--:es-->Por favor, tente responder a esta novamente.<!--:--><!--:pb-->Por favor, tente responder a esta novamente.<!--:-->', 'framework');
			$error = true;
		}
	}
	...
	if(trim($_REQUEST['c_message'] === "")){
		//it's empty
		$message_error = __('<!--:es-->Você esqueceu de preencher a sua mensagem<!--:--><!--:pb-->Você esqueceu de preencher a sua Mensagem<!--:-->', 'framework');
		$error = true;
	}else{
		//it's ok
	...
	if($error != true) {
		...
		$c_subject = __('<!--:es-->Contacte partir do seu site<!--:--><!--:pb-->Contacte partir do seu site<!--:-->', 'framework');
